...
def encrypt_message():
    ...
    if not public_keys:
        ...
        return
    pub_keys_input = []
    pub_key_name = input(f"Choose recipient from {public_keys}: ")
    if pub_key_name in public_keys:
        pub_keys_input.append(pub_key_name)
    else:
        print("that recepient public key aint in this directory")

    if not pub_keys_input:
        print("are you high? choose someone to recieve lmao")
        return

    message = input("Message to encrypt: ")
    try:

        encrypted_msgs = [PKCS1_OAEP.new(RSA.import_key(open(f"{key}_public.key", "rb").read())).encrypt(message.encode()).hex() for key in pub_keys_input]
...
def decrypt_message():
    ...
    if not private_keys:
        print("No private keys available.")
        return
    ...
    try:
        decrypted_message = PKCS1_OAEP.new(private_key).decrypt(bytes.fromhex(encrypted_msgs[0])).decode()
    except ValueError:
